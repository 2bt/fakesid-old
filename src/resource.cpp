...
SDL_Texture* load_texture(char const* name) {
    ...
    if (!img) {
        SDL_Log("could not load image %s", name);
        return nullptr;
    }
...
bool init() {
    if (!(m_textures[TEX_FONT_DEFAULT] = load_texture(ASSET_DIR "font-default.png")) ||
        !(m_textures[TEX_FONT_MONO]    = load_texture(ASSET_DIR "font-mono.png")) ||
        false) return false;
    return true;
}
